...
{{if .Method | isMethodPost}}
	...
	r.ParseForm()
{{ range $p := .ParamInStruct }}{{ if .IsInt}}
...
{{ end }}{{end}}{{else}}{{ range $p := .ParamInStruct }}
...
func main() {
	...
	for i1, h := range apigenapi.Srv {
		for i2, v2 := range h {
			apigenapi.Srv[i1][i2].ParamInStruct =
